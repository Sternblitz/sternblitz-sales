...
export default function DashboardPage() {
  ...
  // Events vom Simulator – aber KEIN Öffnen des Formulars mehr hier!
  useEffect(() => {
    const onStart = (e) => {
      const { name = "", address = "", url = "" } = e.detail || {};
      setGoogleField([name, address].filter(Boolean).join(", "));
      setGoogleUrl(url || "");
      // nicht öffnen – erst nach Klick
    };
    const onOpt = () => {
      const opt = sessionStorage.getItem("sb_selected_option");
      if (opt) setOption(opt);
    ...
    // initiale Werte ziehen (Seite frisch geladen)
    pullLatest();
  ...
  useEffect(() => {
    ...
    try {
      ...
      ac.addListener("place_changed", () => {
        ...
        setGoogleUrl(url || "");
        try { sessionStorage.setItem("sb_selected_profile", JSON.stringify({ name, address, url })); } catch {}
      });
    } catch {}
  }, [formOpen]);
  ...
  const submit = (e) => {
    ...
    const payload = {
      ...
      counts, // nützlich für die nächste Seite (Signatur/PDF)
    };
    try { sessionStorage.setItem("sb_checkout_payload", JSON.stringify(payload)); } catch {}
    console.log("Lead payload (Step 1):", payload);
    alert("Weiter zum nächsten Schritt (Signatur) …");
    // hier später: Router push auf /sign oder ähnlich
  };
    ...
    <main className="sb-wrap">
      ...
      >
        ...
        <form className="lead-form" onSubmit={submit}>
          ...
          {/* Welche Bewertungen sollen gelöscht werden? (+ Counts) */}
          <div className="group">
            <div className="group-title">
      ...
      {/* Styles – nur Relevantes für neue Bits (deine bisherigen Styles können bleiben) */}
      <style jsx global>{`
        ...
        .primary-btn{
          ...
          padding:16px 28px;border-radius:18px;font-weight:800;letter-spacing:.2px;
        ...
        .profile-input input{width:100%;height:36px;border-radius:10px;border:1px solid rgba(0,0,0,.12);
        ...
        .clear-x{position:absolute;right:8px;top:50%;transform:translateY(-50%);width:22px;height:22px;line-height:20px;border-radius:50%;
          border:1px solid rgba(0,0,0,.15);background:#fff;color:#111;font-size:16px;cursor:pointer;padding:0}
        .profile-link{display:inline-flex;align-items:center;height:36px;padding:0 12px;border-radius:10px;border:1px solid #dbeafe;background:#eef5ff;color:#0a58c7;
        ...
        .choice{display:flex;align-items:center;gap:10px;padding:12px 14px;border-radius:12px;border:1px solid #eaf0fe;background:#fff;cursor:pointer;user-select:none;
        ...
        .field input,.field textarea{height:34px;border-radius:10px;border:1px solid rgba(0,0,0,.12);padding:6px 10px;font-size:15px;background:#fff;transition:border-color .16s, box-shadow .16s}
