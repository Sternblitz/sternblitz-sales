...
export async function POST(req: Request) {
  try {
    ...
    let salesRepId: string | null = userId;
    let teamId: string | null = null;
    let sourceAccountId: string | null = userId;

    const { data: profile, error: profileError } = await supabase
      .from("profiles")
      .select("sales_rep_id, team_id, source_account_id")
      .eq("id", userId)
    ...
    if (profile) {
      salesRepId = profile.sales_rep_id ?? userId;
      teamId = profile.team_id ?? null;
      sourceAccountId = profile.source_account_id ?? userId;
    }
    ...
    const { data: pub } = supabase.storage.from("contracts").getPublicUrl(key);
